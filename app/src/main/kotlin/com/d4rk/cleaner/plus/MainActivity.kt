...
class MainActivity : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val preferences = getSharedPreferences("app_usage", MODE_PRIVATE)
    ...
    override fun onResume() {
        ...
        val sharedPreferences = PreferenceManager.getDefaultSharedPreferences(this)
        val preferenceButtonPositions = sharedPreferences.getBoolean(getString(R.string.key_swap_buttons), false)
        swapButtonsPositions(preferenceButtonPositions)
    ...
    private fun scan(delete: Boolean) {
        ...
        reset()
        if (preferences.getBoolean(getString(R.string.key_clipboard), false)) {
            clearClipboard()
        }
        ...
        if (path.listFiles() == null) {
            ...
            runOnUiThread {
                binding.textViewStatus.text =
                    if (delete) getString(R.string.freed) + " " + convertSize(kilobytesTotal)
                    else getString(R.string.found) + " " + convertSize(kilobytesTotal)
                binding.progressBarScan.progress = binding.progressBarScan.max
                binding.textViewPercentage.text = getString(R.string.main_progress_100)
            }
    ...
    private fun requestWriteExternalPermission() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
            requiredPermissions += Manifest.permission.MANAGE_EXTERNAL_STORAGE + Manifest.permission.READ_MEDIA_AUDIO + Manifest.permission.READ_MEDIA_IMAGES + Manifest.permission.READ_MEDIA_VIDEO
            if (!Environment.isExternalStorageManager()) {
                val intent = Intent(Settings.ACTION_MANAGE_APP_ALL_FILES_ACCESS_PERMISSION)
                val uri = Uri.fromParts("package", packageName, null)
                intent.data = uri

...
fun NavigationHost(
    ...
    NavHost(navController = navHostController , startDestination = startupPage) {
        composable(route = com.d4rk.cleaner.core.data.model.ui.navigation.BottomNavigationScreen.Home.route) {
            Box(modifier = Modifier.padding(paddingValues = finalPaddingValues)) {
                HomeScreen()
            }
        }

        composable(route = com.d4rk.cleaner.core.data.model.ui.navigation.BottomNavigationScreen.AppManager.route) {
            Box(modifier = Modifier.padding(paddingValues = finalPaddingValues)) {
                AppManagerScreen()
            }
        }

        composable(route = com.d4rk.cleaner.core.data.model.ui.navigation.BottomNavigationScreen.MemoryManager.route) {
            Box(modifier = Modifier.padding(paddingValues = finalPaddingValues)) {

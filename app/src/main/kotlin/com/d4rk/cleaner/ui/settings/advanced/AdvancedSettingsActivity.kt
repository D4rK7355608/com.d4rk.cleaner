...
class AdvancedSettingsActivity : AppCompatActivity() {
    ...
    class SettingsFragment : PreferenceFragmentCompat() {
        override fun onCreatePreferences(savedInstanceState: Bundle?, rootKey: String?) {
            setPreferencesFromResource(R.xml.preferences_settings_advanced, rootKey)
            val bugReport = findPreference<Preference>(getString(R.string.key_bug_report))
            bugReport?.setOnPreferenceClickListener {
                startActivity(Intent(Intent.ACTION_VIEW, Uri.parse("https://github.com/D4rK7355608/${context?.packageName}/issues/new")))
                true
            }
        }
    }

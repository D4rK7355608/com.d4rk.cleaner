...
fun AboutSettingsComposable(activity : AboutSettingsActivity) {
    ...
    TopAppBarScaffoldWithBackButton(title = stringResource(id = R.string.about) , onBackClicked = { activity.finish() }) { paddingValues ->
        Box(modifier = Modifier.padding(paddingValues)) {
            LazyColumn(
                ...
                item(key = "app_build_version") {
                    PreferenceItem(
                        title = stringResource(id = R.string.app_build_version) , summary = "${BuildConfig.VERSION_NAME} (${BuildConfig.VERSION_CODE})"
                    )
                }
                item(key = "oss_licenses") {
                    PreferenceItem(title = stringResource(id = R.string.oss_license_title) , summary = stringResource(id = R.string.summary_preference_settings_oss) , onClick = {
                        IntentsHelper.openLicensesScreen(
                            context = context , eulaHtmlString = eulaHtmlString , changelogHtmlString = changelogHtmlString
                        )
                    })
                }
                ...
                item(key = "device_info") {
                    ...
                    PreferenceItem(title = stringResource(id = R.string.device_info) , summary = version , onClick = {
                        ClipboardHelper.copyTextToClipboard(context = context , label = "Device Info" , text = version , onShowSnackbar = {
                            showSnackbar = true
                        })
                    })
                }
